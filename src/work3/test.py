# 我后来参考了老师教程的代码运行，尝试运行：
#
# 我的实现与老师给出的实验教程/参考逻辑总体保持一致，核心流程如下：
# 1. 使用 ti.init(arch=ti.gpu) 初始化 GPU 后端；
# 2. 预分配像素缓冲区 pixels、曲线点缓冲区 curve_points_field、
#    控制点对象池 gui_points；
# 3. 在 CPU 端使用 De Casteljau 算法计算贝塞尔曲线采样点；
# 4. 通过 from_numpy(...) 将整批曲线点一次性发送到 GPU；
# 5. 在 GPU Kernel 中完成坐标映射与像素点亮；
# 6. 在主循环中处理鼠标左键添加控制点、键盘 C 键清空画布；
# 7. 使用对象池技巧绘制控制点，并用索引池绘制控制折线。
#
# 与老师教程相比，主要差别在怎么写：
# 1. 代码结构更模块化：
#    将 De Casteljau 算法、像素清空、曲线绘制、主循环交互逻辑
#    分成独立函数，便于阅读和调试。
# 2. 数据流写得更显式：
#    明确区分了 CPU 端的 Python/NumPy 数据与 GPU 端的 Taichi Field，
#    更清楚地体现了“CPU 批量计算 + GPU 并行光栅化”的流程。
# 3. 控制折线部分额外使用了固定大小的索引对象池 gui_indices：
#    这样不仅能绘制控制点，也能稳定绘制控制多边形，贴合完整实验效果。
# 总之，我的实现与老师教程在算法本质、任务完成方式和渲染思想上是一致的。
