...
def render(ka: ti.f32, kd: ti.f32, ks: ti.f32, shininess: ti.f32):
    for i, j in pixels:
        ...
        if obj_id != 0:
            p = ro + rd * t_min
            if obj_id == 1:
                color = blinn_phong_shade(p, sphere_normal(p), SPHERE_COLOR, ka, kd, ks, shininess)
            else:
                color = blinn_phong_shade(p, cone_normal(p, cone_part), CONE_COLOR, ka, kd, ks, shininess)

        pixels[i, j] = color


def main():
    window = ti.ui.Window("Blinn-Phong Shading Demo", (W, H), vsync=True)
    ...
    while window.running:
        ...
        with gui.sub_window("Blinn-Phong Parameters", 0.62, 0.06, 0.34, 0.24):
            gui.text("Half-vector H = normalize(L + V)")
