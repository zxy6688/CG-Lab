...
├── assets/
│   └── work5/
│       ├── optional_tasks.png        # 老师实验文档中的选做内容截图
│       ├── teacher_test.png          # 老师参考代码或测试代码运行结果
...
├── src/
│   └── work5/
│       ├── test.py                   # 老师参考代码或测试代码，用于与自己的实现进行对比
...
其中，$\mathbf{O}$ 表示光线起点，$\mathbf{D}$ 表示单位方向向量，$t$ 表示光线沿方向前进的距离。只有当 $t>0$ 时，交点才位于光线前方。

### 5.2 球体求交

球体由球心 $\mathbf{C}$ 和半径 $r$ 定义，球面上任意一点满足：
...
其中，$\mathbf{V}$ 表示从交点指向相机的视线方向，$s$ 表示高光指数。
...
对于已经命中的表面点 $\mathbf{P}$，程序从该点向光源位置 $\mathbf{L}_{pos}$ 发射阴影射线：
...
其中，$\mathbf{D}$ 是入射光线方向，$\mathbf{N}$ 是表面单位法线，$\mathbf{R}$ 是反射光线方向。
...
其中，$\mathbf{C}_{final}$ 对应程序中的 `final_color`，$\mathbf{T}$ 对应程序中的 `throughput`。
