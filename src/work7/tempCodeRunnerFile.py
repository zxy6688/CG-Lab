...
sphere_radius_field = ti.field(dtype=ti.f32, shape=())
...
def resolve_sphere_collision(pos, vel):
    ...
    dist = offset.norm()
    radius = sphere_radius_field[None]
...
@ti.kernel
def set_sphere(radius: ti.f32):
    sphere_center[0] = ti.Vector([0.10, 0.03, 0.0])
    sphere_radius_field[None] = radius
...
def step_explicit():
    ...
    for i in range(num_particles):
        if is_fixed[i] == 0:
            ...
            x[i], v[i] = resolve_sphere_collision(x[i], v[i])
        else:
    ...
    for _ in ti.static(range(8)):
        ...
        for i in range(num_particles):
            if is_fixed[i] == 0:
                ...
                x_next[i], v_next[i] = resolve_sphere_collision(x_next[i], v_next[i])
            else:
...
def main():
    ...
    set_sphere(current_radius)
    reset_cloth()

    window = ti.ui.Window(
        "Games101 - Optional Collision",
        res=(900, 900),
        vsync=True
    )
    ...
    camera.position(0.10, 0.02, 2.35)
    ...
    while window.running:
        ...
        with gui.sub_window("Control Panel", 0.02, 0.02, 0.39, 0.43):
            gui.text("Optional 2: Sphere Collision")
            gui.text("Cloth particles are projected")
            gui.text("onto the sphere surface")
            gui.text("")
            ...
            gui.text("")
            gui.text("Sphere Radius:")

            if gui.button(("[*] " if current_radius == 0.13 else "[ ] ") + "radius = 0.13"):
                current_radius = 0.13
                set_sphere(current_radius)
                reset_cloth()

            if gui.button(("[*] " if current_radius == 0.15 else "[ ] ") + "radius = 0.15"):
                current_radius = 0.15
                set_sphere(current_radius)
                reset_cloth()

            if gui.button(("[*] " if current_radius == 0.18 else "[ ] ") + "radius = 0.18"):
                current_radius = 0.18
                set_sphere(current_radius)
                reset_cloth()
