...
| 三维立方体透视旋转 | [查看动态演示](#section-7-1-4) | [查看静态效果图](#section-7-1-4) |
...
此外，`test.py` 用于运行老师参考代码或测试版本，方便对比课程示例与本实验实现之间在矩阵构造、坐标变换和可视化效果上的差异。
...
│       ├── cube_demo.png             # 选做一：三维立方体透视旋转静态效果图
...
该文件用于运行老师参考代码，方便观察课程示例中的矩阵变换流程，并与自己的实现进行对比。
...
模型变换描述物体自身在世界空间中的姿态变化。基础任务要求三角形绕 `Z` 轴旋转，设旋转角度为 `theta`，对应的齐次矩阵为：
...
每一帧中，程序对立方体顶点执行与基础三角形相同的 MVP 变换，再遍历 12 条边进行线框绘制。旋转插值版本在两个姿态之间生成中间旋转状态，使立方体姿态变化更加连续。
...
<table align="center">
  ...
  <tr>
    ...
    <td align="center">
      <img src="../../assets/work2/cube_demo.png" alt="Cube Demo Result" width="360">
    </td>
  </tr>
</table>

该组图展示了三维线框立方体的透视旋转效果。程序将基础任务中的三角形扩展为具有 8 个顶点和 12 条边的立方体，并对每个顶点执行相同的 MVP 变换流程。随着立方体绕不同坐标轴旋转，可以明显观察到前后边之间的空间关系和透视投影产生的三维空间感。静态效果图展示了立方体某一帧的透视姿态、顶点标记和界面控制信息。
...
老师补充要求在程序中创建两个不同姿态的立方体，并用旋转插值实现它们之间的变换过渡。该任务的重点是让立方体姿态变化更加连续，而不是只做固定角速度旋转。
...
其中，`theta_0` 表示起始姿态角度，`theta_1` 表示目标姿态角度。本实验在代码中使用四元数 SLERP 来完成旋转插值，使姿态过渡更加自然，避免简单欧拉角插值可能产生的不平滑问题。
...
该组图展示了立方体旋转插值动画。程序设置两个不同的立方体姿态，并使用四元数 SLERP 在两个姿态之间生成连续中间状态。动态演示中，立方体不是简单地固定角速度旋转，而是在两个指定空间朝向之间平滑过渡，因此更符合老师补充要求中“旋转插值”的目标。静态效果图展示了插值过程中某一时刻的中间姿态、坐标轴方向和当前插值参数。
...
本实验完成了老师文档中要求的基础 MVP 变换任务，并进一步完成了三维立方体透视旋转和旋转插值两个选做内容。基础部分实现了 `get_model_matrix(angle)`、`get_view_matrix(eye_pos)` 和 `get_projection_matrix(eye_fov, aspect_ratio, zNear, zFar)` 三个核心函数，并将它们用于真实的三维顶点变换、透视除法和屏幕绘制。
