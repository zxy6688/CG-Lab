...
def blinn_phong_shade(pos, normal, obj_color, ka, kd, ks, shininess):
    ...
    spec_term = 0.0
    if ndotl > 0.0:
        spec_term = ti.pow(ti.max(N.dot(H), 0.0), shininess)

    specular = ks * spec_term * light_color
    ...
    for i, j in pixels:
        ...
        if best_obj != 0:
            ...
            if best_obj == 1:
                ...
                color = blinn_phong_shade(hit_pos, N, sphere_color, ka, kd, ks, shininess)
            else:
                N = cone_normal(hit_pos, cone_part)
                color = blinn_phong_shade(hit_pos, N, cone_color, ka, kd, ks, shininess)

        pixels[i, j] = color


def main():
    window = ti.ui.Window("Work4 Optional 1 - Blinn-Phong", (W, H), vsync=True)
    ...
    while window.running:
        gui = window.get_gui()
        with gui.sub_window("Blinn-Phong Parameters", 0.02, 0.02, 0.32, 0.24):
            gui.text("Specular uses half vector H = normalize(L + V)")
